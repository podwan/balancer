#include "RGB.h"
int threshold_top[3] ={30,30,30};   //触摸阈值
...
void touchInit(int touchID, uint8_t touchPin)
{
  int i = 0;
  float num = 0;
  for(i = 0;i<20;i++)
  ...
  threshold_top[touchID] = num*0.05 -15;
}
//触摸感应处理
void touchAttach(int touchID, uint8_t touchPin) {
  int touchread = touchRead(touchPin);
  if ( touchread <= threshold_top[touchID] ) { //达到触发值的计数
    //delay(38);  // 0.038秒
