...
void read_IMU() {
  ...
  accAngleX = (atan(AccY / sqrt(pow(AccX, 2) + pow(AccZ, 2))) * 180 / PI) + 0.58; // AccErrorX ~(-1.15) See the calculate_IMU_error()custom function for more details
  accAngleY = (atan(-1 * AccX / sqrt(pow(AccY, 2) + pow(AccZ, 2))) * 180 / PI) + 2.57; // AccErrorX ~(0.5)
  ...
  GyroX = GyroX + 1.20; //// GyroErrorX ~(-1.85)     
  GyroY = GyroY - 2.36; // GyroErrorY ~(0.15)
