...
void loop() {  
  ...
  int potValues[numberOfPotSamples];
    ...
    int adjustedValue = map(potValue, 0, 4095, 255, 0);
    ...
    int adjustedValue2 = map(potValue2, 0, 4095, 255, 0);
  ...
  LX_to_send = map_normal(adjustedValue,  0, 135, 255, LX_inverted);
  LY_to_send = map_normal(adjustedValue2, 0, 148, 255, LY_inverted);
  RX_to_send = map_normal(adjustedValue3, 0, 139, 255, RX_inverted);
  RY_to_send = map_normal(adjustedValue4, 0, 154, 255, RY_inverted);
...
//  LX_to_send = LX_to_send  + LX_fine - 127;
//  LY_to_send = LY_to_send  + LY_fine - 127;
//  RX_to_send = RX_to_send  + RX_fine - 127;
//  RY_to_send = RY_to_send  + RY_fine - 127;     
  ...
  data.buttonR3 = digitalRead(R3);
  data.buttonR4 = digitalRead(R4);
  ...
  data.buttonCT = digitalRead(CT);
  data.pot1 = adjustedValue5;
  data.pot2 = adjustedValue6;  
...
//    }    
    ...
    if (digitalRead(L1) == LOW){
      if (digitalRead(L2) == LOW){
        bleGamepad.setAxes(LX_to_send, LY_to_send, 0, 0, RX_to_send, RY_to_send, adjustedValue5,adjustedValue6,DPAD_UP_LEFT);
      } else if (digitalRead(L4) == LOW){
        bleGamepad.setAxes(LX_to_send, LY_to_send, 0, 0, RX_to_send, RY_to_send, adjustedValue5,adjustedValue6,DPAD_UP_RIGHT);
      } else {
        bleGamepad.setAxes(LX_to_send, LY_to_send, 0, 0, RX_to_send, RY_to_send, adjustedValue5,adjustedValue6,DPAD_UP);
      }
    }
    else if (digitalRead(L3) == LOW){
      if (digitalRead(L2) == LOW){
        bleGamepad.setAxes(LX_to_send, LY_to_send, 0, 0, RX_to_send, RY_to_send, adjustedValue5,adjustedValue6,DPAD_DOWN_LEFT);
      } else if (digitalRead(L4) == LOW){
        bleGamepad.setAxes(LX_to_send, LY_to_send, 0, 0, RX_to_send, RY_to_send, adjustedValue5,adjustedValue6,DPAD_DOWN_RIGHT);
      } else {
        bleGamepad.setAxes(LX_to_send, LY_to_send, 0, 0, RX_to_send, RY_to_send, adjustedValue5,adjustedValue6,DPAD_DOWN);
      }
    }
      
    else if (digitalRead(L2) == LOW)
      bleGamepad.setAxes(LX_to_send, LY_to_send, 0, 0, RX_to_send, RY_to_send, adjustedValue5,adjustedValue6,DPAD_LEFT);

    else if (digitalRead(L4) == LOW)
      bleGamepad.setAxes(LX_to_send, LY_to_send, 0, 0, RX_to_send, RY_to_send, adjustedValue5,adjustedValue6,DPAD_RIGHT);
    else 
      bleGamepad.setAxes(LX_to_send, LY_to_send, 0, 0, RX_to_send, RY_to_send, adjustedValue5,adjustedValue6,DPAD_CENTERED); 
...
void gamepadbutton(){
  ...
  int currentButton7State = digitalRead(LB);
  int currentButton8State = digitalRead(RB);
  int currentButton9State = digitalRead(BK);
  int currentButton10State = digitalRead(ST);
  int currentButton11State = digitalRead(CT);
  int currentButton12State = digitalRead(LT);    
  int currentButton13State = digitalRead(RT);    
