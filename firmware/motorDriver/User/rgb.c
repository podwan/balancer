...
uint8_t colors[Led_Num][3];                              // 每个LED有3个颜色通道（RGB）
...
void WS2812_Number_4(uint32_t Color1, uint32_t Color2, uint32_t Color3, uint32_t Color4)
{
    ...
    for (uint8_t k = 0; k < 4; k++)
    {
        ...
        {
        case 0:
            p = (RGB_Buff_4 + Reste_Data) + (0 * Led_Data_Len), Color = Color1;
            break;
        case 1:
            p = (RGB_Buff_4 + Reste_Data) + (1 * Led_Data_Len), Color = Color2;
            break;
        case 2:
            p = (RGB_Buff_4 + Reste_Data) + (2 * Led_Data_Len), Color = Color3;
            break;
        case 3:
            p = (RGB_Buff_4 + Reste_Data) + (3 * Led_Data_Len), Color = Color4;
            break;
        default:;
            break;
        }
    ...
    HAL_TIM_PWM_Start_DMA(&htim2, TIM_CHANNEL_1, (uint32_t *)RGB_Buff_4, 3); // 启动DMA传输
}

void WS2812_Number_1(uint32_t Color1)
{

    uint16_t RGB_Buff_4[Reste_Data + WS2812_Data_Len] = {0};
    uint16_t *p;
    uint32_t Color;
    ...
    HAL_TIM_PWM_Start_DMA(&htim2, TIM_CHANNEL_1, (uint32_t *)colors[0], (176)); // 启动DMA传输
}
//  DMA 传输完成回调函数
